...
async function renderPropertyCoverSlide(data) {
  ...
  const labelW = drawSpacedText(ctx, data.badgeLabel.toUpperCase(), padX, labelY, {
    font: "600 15px 'Archivo', sans-serif", color: '#fff', letterSpacing: 5,
  });
  ...
  if (data.specs && data.specs.length) {
    ...
    data.specs.forEach((spec, i) => {
      ...
      drawSpacedText(ctx, spec.label.toUpperCase(), numX, specTop + 48, {
        font: "600 12px 'Archivo', sans-serif", color: C.labelMuted, letterSpacing: 2,
      });
    });
    y = specTop + 60 + 20;
    ctx.beginPath(); ctx.moveTo(contentX, y); ctx.lineTo(contentX + contentW, y); ctx.stroke();
  ...
  const footerH = 60;
  const priceBlockY = bottomY - footerH - 100;

  ctx.font = "600 13px 'Archivo', sans-serif";
  drawSpacedText(ctx, data.priceLabel.toUpperCase(), contentX, priceBlockY, {
    font: "600 13px 'Archivo', sans-serif", color: C.labelMuted, letterSpacing: 2,
  });
  ctx.font = "800 62px 'Archivo', sans-serif";
  ctx.fillStyle = C.orange;
  ctx.fillText(data.priceValue, contentX, priceBlockY + 58);
  if (data.priceNote) {
    ctx.font = "500 15px 'Archivo', sans-serif";
    ctx.fillStyle = C.labelMuted;
    ctx.fillText(data.priceNote, contentX, priceBlockY + 84);
  }
  ...
  ctx.fillStyle = C.greenDark;
  ctx.textAlign = 'right';
  ctx.fillText(data.cta || 'Agende sua visita →', rightX, priceBlockY);
  ctx.textAlign = 'left';
  ...
  drawSpacedText(ctx, THEME.footerLine, contentX, footerY - 12, {
    font: "600 13px 'Archivo', sans-serif", color: C.labelMuted, letterSpacing: 3,
  });
...
async function renderPropertyCleanSlide(photoSrc) {
  ...
  const [photo, logoWhite] = await Promise.all([
    loadImage(photoSrc),
    loadImage(BRAND_ASSETS.logoWhite),
  ]);
  ...
  const logoW = 340;
  const logoH = logoW * (logoWhite.height / logoWhite.width);
  ctx.save();
  ctx.globalAlpha = 0.92;
  ctx.drawImage(logoWhite, (PROP_W - logoW) / 2, PROP_H - logoH - 90, logoW, logoH);
  ctx.restore();
